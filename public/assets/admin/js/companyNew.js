$(document).ready(function() {
    ...
    'use strict';
    var text_max = 500;
    var url = 'ajaxImageUpload';
    var path = Path+'uploads/employers/company-logos/' ;
    ...
    $('#logoUpload').fileupload({
        ...
        success: function (data) {
            if(data.success === true){
                ...
                $("#header-company-logo").attr("src", path+data.message);
                $(".image-company-profile").attr("src", path+data.message);
                $(".sidebar-logo").attr("src", path+data.message);
                $("#upload-box").pulsate("destroy");
                // $("#logo-error-reporting").html('');

            } else {
                toastr.error(data.message[0], 'Upload error', {timeOut: 5000});
    ...
    $(document).on('click', 'a', function (e) {
        ...
        if(FormCompanyEmployer_orginal != FormCompanyEmployer_new)
        {
            e.preventDefault();
            swal({
                title: "<h5 style='font-size : 18px'>Are you sure to save or continue ?</h5>",
                width: 600,
                padding: 35,
                html: "<button class='btn btn-danger swal_close' data-href='"+href+"'>Cancel</button>&nbsp;"+
                "<button class='btn btn-info build-cv-swal swal_continue'>Yes</button>",
