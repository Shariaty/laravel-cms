...
Invoice.prototype = {
    ...
    calcTotal: function () {
         jQuery($.opt.parentClass).each(function (i) {
             var row = jQuery(this);
             var total = row.find($.opt.price).val() * row.find($.opt.con).val() * row.find($.opt.qty).val();
    ...
    newRow: function () {
        jQuery(".item-row:last").after(
            ...
            '<select class="form-control item product_selector" name="item[]" style="width: 100%;"></select>' +
            ...
            '<td>' +
            '<div class="form-group">' +
            '<input class="form-control price price_value" name="price[]['+Math.random()+']" placeholder="Price" type="number">' +
            '<span class="validation-message-block"></span>' +
            '</div>' +
            '</td>' +
            ...
            '<td>' +
            '<div class="form-group">' +
            '<select class="form-control unit_list" name="unit[]['+Math.random()+']" style="width: 100%;" disabled></select>' +
            '<span class="validation-message-block"></span>' +
            '</div>' +
            '</td>' +
            '<td>' +
            ...
            '<input style="width:130px" class="form-control qty qty_value" name="qty[]['+Math.random()+']" placeholder="Quantity" type="number">' +
            '<span class="conversion"> X 1 </span>' +
            '</div>' +
            '<span style="width: 100%" class="validation-message-block"></span>' +
            '</div>' +
            '</td>' +
            '<td><span class="total mask-price">0</span></td>' +
            '<input class="form-control con" name="con[]['+Math.random()+']" value="1" type="hidden">' +
            '<input class="form-control helperCon" value="1" type="hidden">' +
            '</tr>'
        );
