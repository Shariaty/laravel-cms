...
<span style="font-size: 12px;  color: #a1a1a1;  margin-left: 10px;"><i class="icon-paper-clip icons"></i>&nbsp;
    {{ $type === "sheet" ? 'Technical Sheet' : 'Slide Show media' }}
</span>
...
<div id="logo-thumb-container">
    @if($type === "sheet")
        @if($magazine->sheet)
            <a href="{{route('admin.slide.fileView' , [$magazine , 'type' => "sheet"] )}}" class="resume-upload-link btn btn-xs btn-info" style="margin-left: 3px;" target="_blank">
                <i class="fa fa-eye fa-1x"></i>&nbsp;View
            </a>
            <a href="{{route('admin.slide.removeFile' , ['id' =>  $magazine->id , 'type' => "sheet"])}}" class="resume-upload-link confirmation-remove btn btn-xs btn-danger">
                <i class="fa fa-times"></i>
            </a>
        @endif
    @else
        @if($magazine->file)
            <a href="{{route('admin.slide.fileView' , [$magazine , 'type' => "catalog"])}}" class="resume-upload-link btn btn-xs btn-info" style="margin-left: 3px;" target="_blank">
                <i class="fa fa-eye fa-1x"></i>&nbsp;View
            </a>
            <a href="{{route('admin.slide.removeFile' ,  [$magazine->id ,"catalog"])}}" class="resume-upload-link confirmation-remove btn btn-xs btn-danger">
                <i class="fa fa-times"></i>
            </a>
        @endif
    @endif
